...
def load_grammer(gram_file_name):#OK
    for rule in open(gram_file_name):
        ...
        if len(rhs_symbols) == 1:
            preterm[rhs].append((lhs,prob))#非終端記号の登録
        else:
            nonterm.append((lhs, rhs_symbols[0], rhs_symbols[1], prob))#前終端記号の登録

def add_pre_terminal_symbol(words):#OK
...
def cky(gram_file_name,input_file_name):
    load_grammer(gram_file_name)#OK
